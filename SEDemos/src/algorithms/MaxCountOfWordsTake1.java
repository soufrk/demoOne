...
public class MaxCountOfWordsTake1 {
	...
	/*
	 * for(String token:tokens){ int foundIndex = 0; int count = -1; String
	 * tempCopy = input; do{ //input =
	 * input.indexOf(token)==-1?input:input.substring(input.indexOf(token));
	 * foundIndex = tempCopy.indexOf(token); System.out.println("Found at:"
	 * + foundIndex); tempCopy = tempCopy.substring(tempCopy.indexOf(token)
	 * + 1); System.out.println("Sub-string:" + tempCopy); count++;
	 * }while(foundIndex != -1); System.out.println(token + "," + count);
	 * listOfWordCounts.add(instance.new WordCount2(token, count)); }
	 */

	/*
	 * for(String token:tokens){ Pattern pattern = Pattern.compile(token);
	 * String tempCopy = input; Matcher matcher = pattern.matcher(tempCopy);
	 * System.out.println("Result for - " + token); while (matcher.find()) {
	 * //System.out.println(matcher.start() + "(" + matcher.group() + ")" +
	 * matcher.end()); System.out.println("1st Part:" +
	 * tempCopy.substring(0, matcher.start()+1));
	...
	do {
		...
		// System.out.println(matcher.start() + "(" + matcher.group() +
		// ")" + matcher.end());
		// System.out.println("1st Part:" + tempCopy.substring(0,
		// matcher.start()+1));
