 ...
 * 
 * Build a Stream pipeline that computes the sum of elements and finds the
 * maximum value.
 * 
 * @param args
 */
public class Streams004 {
	...
	Optional<Double> highest = listOfValues.stream().reduce((v1, v2) -> {
	    return v1 > v2 ? v1 : v2;
	});
	System.out.println("Highest Value=" + highest.get());

	Optional<Double> lowest = listOfValues.stream().reduce((v1, v2) -> {
	    return v1 > v2 ? v2 : v1;
	});
