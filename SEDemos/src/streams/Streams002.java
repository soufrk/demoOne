...
public class Streams002 {
	...
	long count = DoubleStream.of(values).map(v -> v).count();
	System.out.println(count);

	// 2. Adding elements using sum()
	double result = DoubleStream.of(values).map(v -> v).sum();
	System.out.println(result);

	// 3. Using explicit reduce method
	OptionalDouble sum = DoubleStream.of(values).reduce(sumOperator);
	if (sum.isPresent()) {
