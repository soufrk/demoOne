...
/**
 * Method reference doesn't work inside filter expression
 * 
 * @author soufrk
 *
 */
public class Collectors01 {
	...
	List<Employee> l = listOfEmployees.stream().peek(System.out::println)
		.filter(calendar.getTime().after(Employee::getDateOfBirth)).collect(Collectors.toList());
	List<Employee> m = listOfEmployees.stream().peek(System.out::println)
		.filter(u -> calendar.getTime().after(u.getDateOfBirth())).collect(Collectors.toList());
