...
/**
 * Multiple element operation Use of findFirst() Vs findAny()
 * 
 * Matching elements in a Stream () Guess the outcome ?
 * 
 * @author soufrk
 */
public class Streams006 {
	...
	System.out.println(listOfEmployees.stream().anyMatch(u -> u.getId() > 2)); // Any match
	System.out.println(listOfEmployees.stream().allMatch(u -> u.getName().startsWith("A"))); // All match
	System.out.println(listOfEmployees.stream().noneMatch(u -> u.getName().trim().length() == 0)); // None match
    }
